...
<div class="card">
    <div class="card-header">
        <h2>Modifier le Véhicule : {{ $vehicule->immatriculation }}</h2>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        <form action="{{ route('vehicules.update', $vehicule->id) }}" method="POST">
            ...
            @method('PUT') {{-- Pour indiquer que c'est une requête PUT (mise à jour) --}}
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="statu" name="statu" required>
                    ...
                    <option value="disponible" {{ old('statu', $vehicule->statu) == 'disponible' ? 'selected' : '' }}>Disponible</option>
                    <option value="en_mission" {{ old('statu', $vehicule->statu) == 'en_mission' ? 'selected' : '' }}>En mission</option>
                    <option value="en_maintenance" {{ old('statu', $vehicule->statu) == 'en_maintenance' ? 'selected' : '' }}>En maintenance</option>
                </select>
            </div>

            <button type="submit" class="btn btn-success">Mettre à Jour le Véhicule</button>
